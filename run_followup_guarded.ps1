...
Write-Output 'Starting guarded follow-up send: --send --no-refresh --limit 50 --min-gap-days 7 --run-cooldown-minutes 10'
& $py 'execute_followups.py' '--send' '--no-refresh' '--limit' '50' '--min-gap-days' '7' '--run-cooldown-minutes' '10'
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Output 'FOLLOWUP_GUARDED_SEND_COMPLETE'
